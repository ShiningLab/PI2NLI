...
def init_args():
    ...
    # rand or nli
    parser.add_argument('--nli_mode', type=str, default='rand')  # none for pi
    ...
    parser.add_argument('--data', type=str, default='parade')
    # model
    # roberta-large
    # albert-xxlarge-v2
    ...
    # training
    # 24, 48 for albert-xxlarge-v2
    # 32, 64 for roberta-large
...
class Config(object):
    ...
    def update_config(self, **kwargs):
        ...
        if 'albert-xxlarge-v2' in self.model:
        ...
        self.CKPT_PATH = os.path.join(
            self.RESOURCE_PATH, 'ckpts', self.method, self.nli_mode, self.data, self.model, str(self.seed)
            )
        ...
        self.PROJECT = 'PI2NLI'
        self.NAME = f'{self.method}-{self.nli_mode}-{self.data}-{self.model}-{self.seed}'
        self.LOG_PATH = os.path.join(
            self.RESOURCE_PATH, 'log', self.method, self.nli_mode, self.data, self.model, str(self.seed)
            )
        ...
        self.RESULTS_PATH = os.path.join(
            self.RESOURCE_PATH, 'results', self.method, self.nli_mode, self.data, self.model,
            )
